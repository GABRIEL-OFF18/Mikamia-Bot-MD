...
let handler = async (m, { conn, usedPrefix }) => {
  ...
  const sections = Object.keys(comandosPorGrupo)
    ...
    .map(groupName => ({
      title: groupName,
      rows: Array.from(comandosPorGrupo[groupName])
        .sort()
        .map(cmd => ({
          title: `\( {usedPrefix} \){cmd}`,
          rowId: `\( {usedPrefix} \){cmd}`,
          description: `Ver comandos de esta categoría`
        }))
    }));
...
${sep}
*Selecciona una categoría abajo 👇*`.trim();
  ...
  // 2. Enviar el Menú de Categorías (List Message)
  await conn.sendMessage(m.chat, {
