...
let handler = async (m, { conn, usedPrefix }) => {
  ...
  Object.values(global.plugins).forEach(plugin => {
    ...
    tags.forEach(tag => {
      ...
      help.forEach(h => {
        if (!/^\$|^=>|^>/.test(h)) {
          comandosPorGrupo[groupName].add(h);
        }
      });
    });
  });

  // Crear botones (uno por categoría)
  let buttons = Object.keys(comandosPorGrupo)
    .sort()
    .map(groupName => {
      // Tomamos el primer comando de la categoría como ejemplo
      const exampleCmd = Array.from(comandosPorGrupo[groupName])[0] || 'menu';
      return {
        buttonId: `\( {usedPrefix} \){exampleCmd}`,
        buttonText: { displayText: groupName },
        type: 1
      };
    });

  // Si hay muchas categorías, limitamos a 10-12 botones (WhatsApp tiene límite)
  if (buttons.length > 12) {
    buttons = buttons.slice(0, 12);
    buttons.push({
      buttonId: `${usedPrefix}menu`,
      buttonText: { displayText: 'Ver más categorías' },
      type: 1
    });
  }
...
*Selecciona una categoría:*`.trim();
  ...
  // 1. Enviar el GIF con encabezado
  ...
  // 2. Enviar botones por categoría
  await conn.sendMessage(m.chat, {
    text: "📋 **Menú por Categorías**",
    footer: packname,
    buttons: buttons,
    headerType: 1   // Importante para que se vean los botones
  }, { quoted: m });
};
