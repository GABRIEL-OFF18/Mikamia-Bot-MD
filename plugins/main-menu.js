...
let handler = async (m, { conn, usedPrefix, text }) => {
  ...
  // Sistema de comandos por categoría
  ...
  const sections = Object.keys(comandosPorGrupo)
    .sort()
    .map(groupName => {
      const rows = Array.from(comandosPorGrupo[groupName])
        .sort()
        .map(cmd => ({
          title: `\( {usedPrefix} \){cmd}`,
          rowId: `\( {usedPrefix} \){cmd}`,
          description: `Comandos de ${groupName.toLowerCase()}`
        }));
...
${sep}
*Selecciona una categoría para ver sus comandos*`.trim();
  ...
  // 1. Enviar el GIF con el encabezado
  ...
  // 2. Enviar el menú interactivo por categorías
  await conn.sendMessage(m.chat, {
